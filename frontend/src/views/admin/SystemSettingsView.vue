          ...
          <div v-else class="space-y-3">
            <div>
              ...
              <p class="text-xs text-gray-400 mt-1">
                Slack 앱 → OAuth &amp; Permissions의 <code class="bg-gray-100 px-1 rounded">xoxb-</code> 토큰.
                <code class="bg-gray-100 px-1 rounded">chat:write</code> 권한이 필요합니다.
                <span v-if="notifyCfg.slackBotTokenStored">비워두면 기존 토큰이 유지되며, 삭제하려면 <code class="bg-gray-100 px-1 rounded">-</code>를 입력하세요.</span>
              </p>
            </div>
            <div>
              ...
              <input v-model="notifyCfg.slackAppToken" type="password" class="input w-full text-sm"
                ...
                autocomplete="new-password" />
              <p class="text-xs text-gray-400 mt-1">
                Socket Mode 활성화 시 발급되는 <code class="bg-gray-100 px-1 rounded">xapp-</code> 토큰
                (<code class="bg-gray-100 px-1 rounded">connections:write</code>). 연결 테스트에 사용합니다.
              </p>
            </div>
            <div>
            ...
            <div class="flex items-start gap-2 p-3 rounded-xl bg-gray-50 border border-gray-200 text-xs text-gray-500">
              ...
              <span>소켓 모드는 외부에서 접근 가능한 URL 없이 Slack 앱을 연동하는 방식입니다.
                알림 <b>발송</b>은 소켓 모드 앱에서도 봇 토큰 기반 Web API(<code class="bg-gray-100 px-1 rounded">chat.postMessage</code>)로 이루어지며,
