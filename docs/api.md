...
세 기능이 **같은 열 순서**를 공유하므로, 내려받은 파일을 그대로 고쳐 다시 올릴 수 있다.
...
| 정책(장) 제목 | 예: `개인정보보호 지침 - 제1장 총칙` | 정책ID·조ID가 없을 때 사용 |
| 조 표기 | 예: `제12조`. 비우면 **장 전체 매핑** | 선택 |
| 조 제목(참고) | 조 제목 | 읽지 않음 |
| 정책ID(선택) / 조ID(선택) | 내보내기 시 자동으로 채워진다 | 있으면 제목보다 **우선** |

##### GET /isms/mappings/export

현재 등록된 매핑 전체를 엑셀(`.xlsx`)로 내려받는다. 시트는 `매핑목록` / `입력규칙` / `정책·조 목록(참고)`.
매핑 1건이 1행이며, 매핑이 없는 항목도 정책 열이 빈 행으로 실려 커버리지를 함께 볼 수 있다.

##### GET /isms/mappings/template

일괄 업로드용 빈 템플릿. 시트 구성은 위와 같고 `매핑입력` 시트의 정책 열이 모두 비어 있다.
`정책·조 목록(참고)` 시트에서 정책ID·조ID를 찾아 복사해 쓸 수 있다.

##### POST /isms/mappings/import *(MANAGER+, multipart/form-data)*

| 파라미터 | 타입 | 설명 |
|----------|------|------|
| `file` | File | `.xlsx` 또는 `.csv` |
| `replace` | boolean | 기본 `false`. `true` 면 **파일에 나온 항목코드의 기존 매핑을 모두 지운 뒤** 파일 내용으로 다시 등록한다(내보낸 파일에서 행을 지워 매핑을 없애는 편집이 그대로 반영된다) |

매핑 대상 지정은 **조ID → 정책ID → 제목** 순으로 해석한다. 조ID가 있으면 소속 장은 조에서 따라간다.
정책 열이 모두 빈 행은 `매핑 없음`으로 보고 건너뛴다(`total` 에도 세지 않는다).
이미 등록된 매핑은 중복 등록하지 않고 `skipped` 로 센다.

```json
// Response
{
  "total": 240, "success": 12, "skipped": 228, "failed": 1,
  "removed": 0, "clearedItems": 0,
  "errors": [{ "row": 31, "itemCode": "1.1.1", "message": "정책(장)을 찾을 수 없습니다: 없는 지침 - 제1장" }]
}
```
...
### GET /reports/security-reviews/:id/pdf

심의 **완료(`COMPLETED`)** 건의 결과 보고서 PDF(세로 A4). 심의가 끝나기 전(`REQUESTED`·`IN_REVIEW`·`REVISION`)에는
결과·심의자가 확정되지 않아 `400` (`심의가 완료된 건만 보고서를 만들 수 있습니다.`) 으로 막는다.

| 파라미터 | 타입 | 설명 |
|----------|------|------|
| `lang` | string | `ko`(기본) / `en` |

구성: 표지(제목·회사명·시스템명·심의일) → **심의 결과 배너**(승인 초록 / 조건부 승인 노랑 / 반려 빨강) →
`1. 심의 개요`(제목·시스템·구분·부서·요청자·요청일·오픈예정일·첨부·개인정보 처리·외부 공개 + 구축·변경 개요) →
`2. 검토 결과 요약`(항목·적합·부적합·해당없음·미검토 타일과 검토 영역별 표) →
`3. 설계 검토 체크리스트`(영역·항목·기준·결과·의견) → `4. 심의 의견` → 심의자·심의일시.

다른 보고서와 달리 **로그인한 사용자면 받을 수 있다** — 같은 내용을 심의 상세 화면에서 이미 볼 수 있으므로
열람 권한과 맞춘 것이다.
