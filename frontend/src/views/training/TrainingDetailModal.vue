              ...
              <div v-for="q in wrongQuestions" :key="q.id">
                ...
                <div class="space-y-1.5">
                  <div v-for="opt in options(q)" :key="opt.key"
                    ...
                    :class="isCorrectOption(q.correctAnswer, opt.key) ? 'border-green-500 bg-green-50'
                      : isCorrectOption(q.myAnswer, opt.key) ? 'border-red-400 bg-red-50' : 'border-gray-200'">
                    <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 text-xs font-bold"
                      :class="isCorrectOption(q.correctAnswer, opt.key) ? 'border-green-500 bg-green-500 text-white'
                        : isCorrectOption(q.myAnswer, opt.key) ? 'border-red-400 bg-red-400 text-white' : 'border-gray-300 text-gray-400'">
                      {{ opt.key }}
                    </div>
