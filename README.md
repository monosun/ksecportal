...
| **알기쉬운 보안사례집** | 어려운 보안 개념을 실제 업무 상황으로 풀어 놓은 사례 모음 — **전 임직원용 11개 사례**(피싱·비밀번호 재사용·MFA·세션 탈취·권한 분리·인증과 인가·API Key·로그 속 개인정보·SQL 인젝션·생성형 AI)와 **경영진 보안 투자 36개 항목**(투자 질문 15선·예산 확보 7원칙·인력 확보 논리·이사회 1페이지 템플릿) 2개 탭, 카드 클릭 시 상세 팝업(←/→ 앞뒤 이동), 제목·본문 통합 검색. 본문은 `docs/security-cases-*.md` 가 원본(DB 사용 안 함) |
