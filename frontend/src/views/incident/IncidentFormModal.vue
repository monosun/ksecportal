  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
    ...
    <div class="relative bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[92vh] flex flex-col">
      <div class="flex items-center justify-between px-5 py-3 border-b shrink-0">
        <h2 class="text-lg font-semibold text-gray-900">{{ isEdit ? '인시던트 수정' : $t('incident.create') }}</h2>
      ...
      <form id="incidentForm" @submit.prevent="submit" class="px-5 py-4 overflow-y-auto flex-1 space-y-4">
        <div>
        ...
        <div class="relative" ref="assigneeDropdownRef">
          ...
          <div class="relative">
            <input v-model="assigneeSearch" type="text" class="input w-full pr-8"
              placeholder="담당자 이름 검색 또는 직접 입력"
              @input="onAssigneeInput" @focus="showAssigneeDropdown = true"
              ...
              @keydown.esc="showAssigneeDropdown = false" autocomplete="off" />
        ...
        <div>
          ...
          <textarea v-model="form.description" rows="4" class="input w-full resize-none" placeholder="인시던트 경위, 영향 범위, 초기 대응 내용 등을 기록하세요" />
        </div>
      ...
      <div class="flex justify-end gap-3 px-5 py-3 border-t shrink-0">
        ...
        <button type="submit" form="incidentForm" :disabled="submitting" class="btn-primary text-sm">
...
function emptyForm() {
  return { title: '', severity: '', type: '', affectedSystems: '', description: '', detectedAt: '', assigneeId: null, assigneeText: '' }
}
...
watch(() => props.open, async (open) => {
  ...
  error.value = null
  if (!usersLoaded) {
    try { allUsers.value = (await adminApi.listUsersSimple()).data || [] } catch { allUsers.value = [] }
    usersLoaded = true
  }
  if (props.editId) {
    try {
      const i = (await incidentApi.get(props.editId)).data
      form.value = {
        title: i.title, severity: i.severity, type: i.type,
        affectedSystems: i.affectedSystems || '', description: i.description || '',
        detectedAt: i.detectedAt ? i.detectedAt.slice(0, 16) : '',
        assigneeId: i.assigneeId || null, assigneeText: ''
      }
      assigneeSearch.value = i.assigneeName || ''
    } catch (e) { error.value = e || '불러오기에 실패했습니다.' }
  } else {
...
async function submit() {
  ...
  error.value = null
  try {
    const payload = { ...form.value, detectedAt: form.value.detectedAt || null }
    if (!payload.assigneeId) delete payload.assigneeId
    if (!payload.assigneeText) delete payload.assigneeText
    if (isEdit.value) await incidentApi.update(props.editId, payload)
    else await incidentApi.create(payload)
    emit('saved')
  } catch (e) {
    error.value = e
  } finally {
