...
> **최신 버전: v1.10.0** — 개인정보보호 메뉴 14개로 전면 확장(처리현황·파일관리·수집이용·제공·보유기간·파기·DPIA·유출관리·권리행사·보호조치·현황보고서 11개 신설), Spring Boot 3.3.5(EOL) → 3.5.16 보안 업그레이드, 환경설정 정보 탭 기술스택 버전 자동 조회 ([릴리즈 노트](release/v1.10.0/RELEASE_NOTES.md))
...
| **개인정보보호 — 처리현황·파일관리** | 개인정보 처리업무 등록(처리부서·목적·항목·보유기간·처리근거·처리시스템·업무흐름도·라이프사이클), 개인정보파일대장(DB 테이블·담당부서·민감정보/고유식별정보 포함 여부·보유 건수) |
